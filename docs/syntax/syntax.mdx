...
This article demonstrates the **snapblocks syntax**.

On the [Snap Forum](https://forum.snap.berkeley.edu), code must be written between `[snapblocks]...[/snapblocks]` tags. (On the wiki, `<snapblocks>...</snapblocks>` tags are used.).
...
Arguments, or inputs to a block, are represented
...
String inputs are created with square brackets:
`[lorem ipsum]`
...
first, it will appear as a variable.
...
Scratch Modifications and old or

...
paintBucket bucket @paintBucket :: other
eraser @eraser :: other
...
However, you cannot put multiple inputs vertically on the same line, like in the <SnapBlocks inline="true">{`(pipe [] @arrowRight (() @verticalEllipsis @addInput))`}</SnapBlocks> block.
